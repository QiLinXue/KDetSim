...
void KGeometry::GetGrid(TH3I *x, Short_t which)
{
  ...
  if (DM != NULL)
    if (DM->GetNbinsX() != nx)
      printf("Warning: dimenssions mismatch - X !\n");
  if (DM != NULL)
    if (DM->GetNbinsY() != ny)
      printf("Warning: dimenssions mismatch - Y !\n");
  if (DM != NULL)
    if (DM->GetNbinsZ() != nz)
      printf("Warning: dimenssions mismatch - Z !\n");
      ...
      for (int i = 1; i <= nx; ++i)
      {
        ...
        if (!(cval & 1 || cval & 2 || cval >= 32768))
        {
          ...
          if (k > 1)
          {
            int val = EG->GetBinContent(i, j, k - 1);
            if (val & 1 || val & 2 || val >= 32768)
            {               // k is next to external electrode
              nval |= 1024; // GK
...
Float_t KGeometry::GetLowEdge(Int_t dir)
{
  ...
  {
  case 0:
    ret = EG->GetXaxis()->GetBinLowEdge(1);
    break;
  case 1:
    ret = EG->GetYaxis()->GetBinLowEdge(1);
    break;
  case 2:
    ret = EG->GetZaxis()->GetBinLowEdge(1);
    break;
  default:
    printf("Index %i out of scope!\n", dir);
    ret = 0;
    break;
  }
  return ret;
...
Float_t KGeometry::GetUpEdge(Int_t dir)
{
  ...
  {
  case 0:
    ret = EG->GetXaxis()->GetBinUpEdge(nx);
    break;
  case 1:
    ret = EG->GetYaxis()->GetBinUpEdge(ny);
    break;
  case 2:
    ret = EG->GetZaxis()->GetBinUpEdge(nz);
    break;
  default:
    printf("Index %i out of scope!\n", dir);
    ret = 0;
        ...
        for (j = -(Int_t)(W[1] / GetStepSize(1, By)) + By; j <= (Int_t)(W[1] / GetStepSize(1, By)) + By; ++j)
          if (j <= ny && j >= 1)
            for (q = -(Int_t)(W[0] / GetStepSize(0, Bx)) + Bx; q <= (Int_t)(W[0] / GetStepSize(0, Bx)) + Bx; ++q)
              ...
              {
                if (EG != NULL)
                  EG->SetBinContent(q, j, k, Wei);
                if (DM != NULL)
                  DM->SetBinContent(q, j, k, Mat);
              }
      } // k, j, q
  ...
  for (k = zpl; k <= zpr; ++k)
    for (j = ypl; j <= ypr; ++j)
      for (i = xpl; i <= xpr; ++i)
      {
        ...
        //	  printf("%d %d %d %d\n",i,j,k,Wei);
        if (EG != NULL)
          EG->SetBinContent(i, j, k, Wei);
        if (DM != NULL)
          DM->SetBinContent(i, j, k, Mat);
      }
}
  ...
  for (k = 1; k <= nz; ++k)
    ...
    for (j = 1; j <= ny; ++j)
      ...
      for (i = 1; i <= nx; ++i)
      {
        ...
        {
        case 3:
          D = TMath::Sqrt(TMath::Power(x - Pos[0], 2) + TMath::Power(y - Pos[1], 2)) - R;
          break;
        case 2:
          D = TMath::Sqrt(TMath::Power(x - Pos[0], 2) + TMath::Power(z - Pos[2], 2)) - R;
          break;
        case 1:
          D = TMath::Sqrt(TMath::Power(y - Pos[1], 2) + TMath::Power(z - Pos[2], 2)) - R;
          break;
        }
