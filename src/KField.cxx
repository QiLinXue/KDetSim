...
Float_t KInterpolate2D(TH3D *his, Float_t x, Float_t y, Int_t dir, Int_t bin)
{
  ...
  {
  case 3:
    v11 = his->GetBinContent(EX1, EY1, bin);
    v21 = his->GetBinContent(EX2, EY1, bin);
    v22 = his->GetBinContent(EX2, EY2, bin);
    v12 = his->GetBinContent(EX1, EY2, bin);
    break;
  case 2:
    v11 = his->GetBinContent(EX1, bin, EY1);
    v21 = his->GetBinContent(EX2, bin, EY1);
    v22 = his->GetBinContent(EX2, bin, EY2);
    v12 = his->GetBinContent(EX1, bin, EY2);
    break;
  case 1:
    v11 = his->GetBinContent(bin, EX1, EY1);
    v21 = his->GetBinContent(bin, EX2, EY1);
    v22 = his->GetBinContent(bin, EX2, EY2);
    v12 = his->GetBinContent(bin, EX1, EY2);
    break;
  }
                          ...
                          Float_t Charg, Double_t Neff, Int_t which)
{
  ...
  CalFieldXYZ(cx, cy, cz, E);
  E[0] *= 1e4;
  std::cout << "Charge: " << Charg << ", Mobility: " << Mobility(E[0], T, Charg, Neff, which) << std::endl;
...
TH2D *KField::Draw(Char_t *opt, Int_t b1, Int_t b2)
{
  TH2D *ret = NULL;
  if (!strcmp("U", opt))
    ret = KHisProject(U, b1, b2);
  if (!strcmp("E", opt))
    ret = KHisProject(E, b1, b2);
  if (!strcmp("X", opt))
    ret = KHisProject(Ex, b1, b2);
  if (!strcmp("Y", opt))
    ret = KHisProject(Ey, b1, b2);
  if (!strcmp("Z", opt))
    ret = KHisProject(Ez, b1, b2);
  return ret;
}
...
Float_t KField::M(Int_t dir, Float_t a1, Float_t a2, Float_t a3)
{
  ...
  Int_t NumBin;
  Int_t p1 = E->GetXaxis()->FindBin(a1);
  NumBin = E->GetNbinsX();
  Int_t p2 = E->GetYaxis()->FindBin(a2);
  NumBin = E->GetNbinsY();
  Int_t p3 = E->GetZaxis()->FindBin(a3);
  NumBin = E->GetNbinsZ();
